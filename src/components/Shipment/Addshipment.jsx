            ...
            <div className={`col-span-1 md:col-span-5 h-full flex flex-col p-5 w-full ${currentLanguage === 'ar' ? 'text-right' : ''}`}>
                <div className={`flex flex-col md:flex-row justify-evenly mt-4 ${currentLanguage === 'ar' ? 'md:flex-row-reverse' : ''}`}>
                    <div className="w-full md:w-1/4 text-center mb-4 md:mb-0 md:ml-10 border-b-2 border-red-900">
                        <p className="text-lg md:text-2xl font-bold text-custom-red">{t('addShipment.title')}</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className='md:px-24 mt-10'>
                    <div className="flex justify-start mb-5">
                        <div className='w-2/3'>
                            <label className="block mb-2 text-2xl font-semibold">
                                {t('addShipment.ShipmentImage')} <span className="text-sm text-gray-500 font-light">{t('addRequest.onlyOneImage')}</span>
                            </label>
                            <div
                            ...
                            >
                                {uploadedImage ? (
                                    ...
                                    <span>{t('addShipment.dropImageOr')} <label htmlFor="imageUpload" className="primary-color cursor-pointer">{t('addRequest.browse')}</label></span>
                                )}
                                <input
                                    id="imageUpload"
                                    type="file"
                    ...
                    <div className="flex justify-between mb-5">
                        <input type="text" placeholder={t('addShipment.supplieraddress')} className="w-1/2 p-2 border rounded-2xl border-gray-400 mr-2" />
                        <input type="text" placeholder={t('addShipment.Recivingaddress')} className="w-1/2 p-2 border rounded-2xl border-gray-400" />
                    </div>

                 

                    <textarea placeholder={t('addShipment.note')} className="w-full p-2 border rounded-2xl border-gray-400 mb-5"></textarea>

                    <button type="submit" className="primary-bg text-white p-4 text-lg w-1/2 m-auto rounded-xl block">
                        {t('addShipment.confirmShipment')}
                    </button>
                </form>
            </div>
        </div>

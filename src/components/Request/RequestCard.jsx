...
function RequestCard({ request, onDelete }) {
    ...
    return (
        <div 
            key={request._id} 
            className={`flex flex-col md:flex-row justify-between items-center bg-white shadow-lg p-4 mb-4 rounded-2xl w-full ${currentLanguage === 'ar' ? 'md:flex-row-reverse' : ''}`}
        >
            <div className='py-5 px-2 w-full md:w-1/2'>
                ...
                <p className='text-[#4B4B4B]'>
                    {new Date(request.createdAt).toLocaleString(currentLanguage === 'ar' ? 'ar-EG' : 'en-US')}
                </p>
                {request.link && (
            ...
            <div className='min-w-full md:min-w-80 flex flex-col items-center px-2 md:px-16 mt-4 md:mt-0'>
                ...
                />
                <div className={`flex flex-col md:flex-row items-center justify-between w-full ${currentLanguage === 'ar' ? 'md:flex-row-reverse' : ''}`}>
                    <Link to={`/request/details/${request._id}`} className="primary-color text-lg md:text-xl mb-2 md:mb-0">
                        {t('Details')}
                    </Link>
                    <button 
                        className={`primary-color text-lg md:text-xl flex items-center space-x-2 ${currentLanguage === 'ar' ? 'flex-row-reverse' : ''}`} 
